...
Disassembler::Disassembler(MCDirector *NewMC, object::ObjectFile *NewExecutable,
  ...
  {
    object::SymbolRef::Type sym_type;
    if(error_code ec = sym_iter->getType(sym_type))
      errx(-1, "get sym's type failed: %s", ec.message().c_str());
    if(sym_type != object::SymbolRef::Type::ST_Function)
      continue;
    object::section_iterator sec_iter = Executable->end_sections();
    if(error_code ec = sym_iter->getSection(sec_iter))
      errx(-1, "get sym's sec failed: %s", ec.message().c_str());
    bool is_text;
    if(error_code ec = sec_iter->isText(is_text))
      errx(-1, "get sec's isText failed: %s", ec.message().c_str());
    if(!is_text)
      continue;
    StringRef sym_name;
    if(error_code ec = sym_iter->getName(sym_name))
      errx(-1, "get sym's name failed: %s", ec.message().c_str());
    outs() << "func sym: " << sym_name;
    uint64_t sym_addr;
    if(error_code ec = sym_iter->getAddress(sym_addr))
      errx(-1, "get sym's addr failed: %s", ec.message().c_str());
    outs() << ": 0x";
...
MachineFunction* Disassembler::disassemble(unsigned Address) {
  ...
  if (MF->size() == 0) {
    ...
    do {
      unsigned MBBSize = 0;
      MBB = decodeBasicBlock(Address+Size, MF, MBBSize, nextAddr);
      Size += MBBSize;
    } while (Address+Size < CurSectionEnd && MBB->size() > 0
      /*&& !(MBB->instr_rbegin()->isReturn())*/&& MBB->instr_rbegin()->getDebugLoc().getLine()+MBB->instr_rbegin()->getDesc().getSize()<nextAddr);
...
MachineBasicBlock* Disassembler::decodeBasicBlock(unsigned Address,
  MachineFunction* MF, unsigned &Size, unsigned nextAddr) {
...
//  uint64_t Off = Address-MFLoc;
  ...
  while (Address+Size < nextAddr) {
    ...
    if (MI != NULL && MI->isTerminator() && MI->getOpcode()!=X86::CALL64m) {
      break;
    }
  }
...
DebugLoc* Disassembler::setDebugLoc(uint64_t Address) {
  ...
  Elts->push_back(ConstantInt::get(Int64, dwarf::DW_TAG_lexical_block));
...
#if LLVM_VERSION_CODE == LLVM_VERSION(3, 4)
  error_code ec;
  error_code sym_iter_ec;
  for (object::symbol_iterator I = Executable->begin_symbols(),
         E = Executable->end_symbols(); !sym_iter_ec && I != E; I = I.increment(sym_iter_ec)) {
#else
  std::error_code ec;
...
#if LLVM_VERSION_CODE == LLVM_VERSION(3, 4)
  assert(!sym_iter_ec && "fail when iter sym in Disassembler::getSymbolName");
#endif
  return "";
}
...
#if LLVM_VERSION_CODE == LLVM_VERSION(3, 4)
  error_code sec_iter_ec;
  for (object::section_iterator seci = Executable->begin_sections(); !sec_iter_ec && seci !=
      Executable->end_sections(); seci = seci.increment(sec_iter_ec))
  {
    error_code rel_iter_ec;
    for (object::relocation_iterator ri = seci->begin_relocations(); !rel_iter_ec && ri != 
         seci->end_relocations(); ri = ri.increment(rel_iter_ec)) {
#else
...
#if LLVM_VERSION_CODE == LLVM_VERSION(3, 4)
    assert(!rel_iter_ec && "fail when iter rel in Disassembler::getRelocFunctionName");
  }
  assert(!sec_iter_ec && "fail when iter sec in Disassembler::getRelocFunctionName");
#else
  }
#endif
...
#if LLVM_VERSION_CODE == LLVM_VERSION(3, 4)
  assert(!sym_iter_ec && "fail when iter sym in Disassembler::getFunctionName");
#endif
...
void Disassembler::setSection(const object::SectionRef Section) {
  ...
  assert(!ec && "get addr failed in Disassembler::setSection");
  ec = Section.getSize(SectSize);
  assert(!ec && "get size failed in Disassembler::setSection");
#else
...
const object::SectionRef Disassembler::getSectionByName(StringRef SectionName)
  ...
  for (object::section_iterator si = Executable->begin_sections(), se =
         Executable->end_sections(); !sec_iter_ec && si != se; si = si.increment(sec_iter_ec)) {
#else
  std::error_code ec;
  for (object::section_iterator si = Executable->section_begin(), se =
    ...
    if (si->getName(Name)) {
      ...
      assert(!ec && "get addr failed in Disassembler::getSectionByName");
#else
      Addr = si->getAddress();
#endif
...
#if LLVM_VERSION_CODE == LLVM_VERSION(3, 4)
  assert(!sec_iter_ec && "fail when iter sec in Disassembler::getSectionByName");
  for (object::section_iterator si = Executable->begin_sections(), se =
         Executable->end_sections(); !sec_iter_ec && si != se; si = si.increment(sec_iter_ec)) {
#else
  for (object::section_iterator si = Executable->section_begin(), se =
...
#if LLVM_VERSION_CODE == LLVM_VERSION(3, 4)
      ec = si->getAddress(SectionAddr);
      assert(!ec && "get addr failed in Disassembler::getSectionByName");
      ec = si->getSize(SectionSize);
      assert(!ec && "get size failed in Disassembler::getSectionByName");
#else
...
#if LLVM_VERSION_CODE == LLVM_VERSION(3, 4)
  assert(!sec_iter_ec && "fail when iter sec in Disassembler::getSectionByAddress");
  return *Executable->end_sections();
#else
  return *Executable->section_end();
#endif
